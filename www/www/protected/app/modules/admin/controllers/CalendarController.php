...
class CalendarController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$model->EventStartDate = date('Y-m-d');

		if (isset($_POST['Articles']) && isset($_POST['Events']) ) {
			...
			if ($article->validate()) {
				...
				if ($model->validate()) {
					...
					if ($result) {
						$transaction->commit();
						$this->redirect(array('update','id'=>$model->EventId));
					}
...
// 			Yii::trace(CVarDumper::dumpAsString($model),'vardump');
	...
	{
		$model=Events::model()->with('article')->findByPk($id);
		if($model===null)
